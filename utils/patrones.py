...
class GeneradorPatrones:
    ...
    def generar_patron(self):
        ...
        self.tipo_actual = random.choice(["circulo", "triangulo", "curva", "zigzag", "espiral"])
        puntos = []
        puntos_clave = []
        ...
        self.puntos_clave = puntos_clave if puntos_clave else [puntos[0], puntos[len(puntos)//2], puntos[-1]]
    ...
    def calcular_progreso_detallado(self, rastro_usuario):
        ...
        cobertura = len(puntos_patron_cubiertos) / len(self.patrones) if self.patrones else 0
        ...
        porcentaje_patron = (max_indice_alcanzado / len(self.patrones)) * 100 if self.patrones else 0
    ...
    def dibujar_patron_en_frame(self, frame, rastro_usuario=[]):
        ...
        for i, punto_clave in enumerate(self.puntos_clave):
            if any(math.sqrt((p[0]-punto_clave[0])**2 + (p[1]-punto_clave[1])**2) < umbral for p in rastro_usuario):
